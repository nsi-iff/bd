...
feature 'mudar papel do usuário' do
  ...
  scenario 'administrador de instituição pode gerenciar apenas usuários de sua instituição' do
    ...
    usuario = Factory.create(:usuario, campus: camp1)
    usuario.papeis << Papel.instituicao_admin
    Factory.create(:usuario, nome_completo: 'Rodrigo', campus: camp1)
    Factory.create(:usuario, nome_completo: 'Priscila', campus: camp2)
    autenticar(usuario)
