...
describe Arquivo do
  ...
  it 'verifica mimetype para um upload_audio_ogg' do
    subject.uploaded_file = ActionDispatch::Http::UploadedFile.new({
      tempfile: File.new('spec/resources/audio.ogg')
    })
  ...
  it "quando for criado, envia o arquivo ao sam" do
    ...
    ServiceRegistry.sam.should_receive(:store).
      with(file: arquivo.content_base64, filename: 'arquivo.rtf').
      and_return(double("key" => "123"))
    arquivo.save
