...
(function($) {
    ...
    $.configureBoxes = function(options) {
        ...
        var index = settings.push({
            box1View: 'objeto_de_aprendizagem_curso_ids',
    ...
    function RemoveSelected(removeGroup, otherGroup) {
        $('#' + removeGroup.view + ' option:selected').remove();
    }

    function RemoveAll(removeGroup, otherGroup) {
            $('#' + removeGroup.storage + ' option').clone().addClass('copiedOption').add('#' + removeGroup.view + ' option').remove();
    }
    ...
    function IsMoveMode(currSettings) {
        return currSettings.transferMoe == 'move';
    }
})(jQuery);
